...
app.get('/about', function(req,res){
    ...
    Books.countDocuments().exec()
      .then(count=>{
        console.log("Total documents Count before addtion:",count)
      }) .catch(err=>{
        console.error(err)
      })
})
app.get('/allbooks1',async(req,res)=>{
    const id=await Books.find();
    return res.json(id)
})
...
app.post('/addbooks', function(req,res){
    ...
    newbook.save()
    .then(todo=>{
        res.status(200).json({'books':'new book added successfully'});
    })
    .catch(err=>{
...
app.post('/updatebook/:id', function(req,res){
    ...
    Books.findByIdAndUpdate(id,
        {
        booktitle:updatedbook.booktitile,
